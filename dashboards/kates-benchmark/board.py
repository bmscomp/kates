...
def _header() -> list[dict]:
    return [
        ...
        P.stat(
            ...
            P.targets("sum(kates_benchmark_records_total{%s}) or vector(0)" % SEL),
            unit="short", thresholds=GREEN,
        ),
        P.stat(
            "Errors",
            ...
            "is an average over a run that was partly not running.",
            P.targets(
                "sum(kates_benchmark_errors_total{%s}) or vector(0)" % SEL),
        ...
        P.stat(
            ...
            "violated in this run is never registered, which is why the "
            "`or vector(0)` is load-bearing: with no violations there is no "
            "series at all.",
            P.targets("sum(kates_benchmark_sla_violations{%s}) or vector(0)" % RUN),
            unit="short", thresholds=GREEN_RED_1,
        ),
    ]
