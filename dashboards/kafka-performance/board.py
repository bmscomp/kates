...
# The Strimzi operator's own boards paint "Brokers Online" red at zero and
# this repository painted it green, on the same cluster, at the same moment.
...
def _header() -> list[dict]:
    return [
        P.stat(
            ...
            "gauge. Red at zero, and deliberately so: on a panel counting "
            "things that must exist, zero is the outage and not the resting "
            "state. The bare `or vector(0)` is what draws that zero when "
            "every broker has stopped answering — the one place on this "
            "board where an absent series really does mean none.",
        ...
        P.stat(
            ...
            "above zero for 5 minutes. The zero fallback is anchored to the "
            "brokers' own leader count, because on this panel more than any "
            "other a green zero that means 'nothing is being scraped' would "
            "be read as 'the run is clean'.",
