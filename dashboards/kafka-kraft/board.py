...
def _header() -> list[dict]:
    return [
        ...
        P.stat(
            ...
            P.targets("sum(kafka_server_raftmetrics_number_unknown_voter_connections"
                      "{%s}) or vector(0)" % SEL),
        ...
        P.stat(
            ...
            "that is down, and no pod-level signal distinguishes them.",
            P.targets("max(kafka_controller_kafkacontroller_fencedbrokercount{%s}) "
                      "or vector(0)" % SEL),
            unit="short", thresholds=GREEN_RED_1,
        ),
        P.stat(
            ...
            "Quorum and metadata splits them into which kind.",
            P.targets(
                "(sum(kafka_server_brokermetadatametrics_metadata_load_error_count{%s}) "
                "or vector(0)) + "
                "(sum(kafka_server_brokermetadatametrics_metadata_apply_error_count{%s}) "
                "or vector(0)) + "
                "(sum(kafka_controller_kafkacontroller_metadataerrorcount{%s}) "
                "or vector(0))" % (SEL, SEL, SEL)),
            unit="short", thresholds=GREEN_RED_1,
        ),
    ]
