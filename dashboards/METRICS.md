...
**213 distinct series across twelve boards.** Nothing here was typed from a JMX
...
The most useful question this file answers. Of the 213 series, **136 are
...
**The MirrorMaker 2 boards scope on `$namespace` and `$cluster`**, the same
`label_values` dropdowns the Kafka and Connect boards use, resolved from
`kafka_connect_worker_rebalance_metrics_completed_rebalances_total`. They were
hidden `constant`s the chart filled in per release until phase 1 put
`kafka-common.strimziRelabelings` on the MirrorMaker PodMonitor; with
`strimzi_io_cluster` on every series a worker publishes there is nothing left
for a constant to hold, and `$pods` — a pod-name regex built from the release
name — is gone entirely. The chart still writes each variable's `current`, so
a chart-delivered board opens on its own release without naming it in the
query.

One exception on those boards, and it is a label name rather than a value: the
**recorded `mm2:*` series carry `cluster`, not `strimzi_io_cluster`**. The SLI
rules aggregate with `max by (namespace)`, which drops every label the scrape
added, and the PrometheusRule then attaches `cluster` and `source` itself. The
string is the same — the KafkaMirrorMaker2's name — so the SLO row selects
`namespace="$namespace", cluster="$cluster"` while every other panel selects
...
## The chaos recording rules: `kafka:chaos:*`

`charts/monitoring/templates/prometheus-chaos-rules.yaml` defines eight
recording rules in the `kafka-chaos-rto-rpo` group. Until this release **every
one of them read a series nothing in this repository published**, so all of
them were permanently empty and the four alerts sitting on them
(`KafkaRTOExceedsSLA`, `KafkaRPOExceedsSLA`, `KafkaDataLossDetected`,
`KafkaE2ELatencySpike`) installed cleanly and could never fire. The file also
contained `max without(instance) (a, b)`, which is not valid PromQL, so
Prometheus would have refused the whole `PrometheusRule` and loaded none of its
four groups — including the three that were correct. Nothing had ever rendered
the file, because it lives behind `chaosAlerts.enabled`, which defaults to
`false`.
...
The last two used to read `kafka_chaos_e2e_latency_ms` and
`kafka_chaos_producer_throughput_records_per_sec`. Those names were never
published and would have been misplaced if they had been — `kafka_` is the JMX
exporter's namespace, and both are the workload's own measurements, which Kates
already reports. So rather than registering duplicate meters under a second
name, the rules now read the series that exist.

**These rules fill only for a run that verified integrity.** `BenchmarkMetrics`
registers the seven `kates_integrity_result_*` gauges lazily, on the first
verification a run reports, so a plain load test publishes none of them. An
empty panel on the `kates-chaos` RTO/RPO row therefore means "integrity was not
verified", not "recovery was instant" — which is exactly the distinction a zero
would destroy.

### A unit trap worth knowing about

`IntegrityResult` holds these as `java.time.Duration` and exposes
**milliseconds**; the series names and every alert threshold are in
**seconds**. `BenchmarkMetrics.recordIntegrity` divides by 1000 before
publishing. Had it not, `KafkaRTOExceedsSLA` — which fires above 30, meaning
thirty seconds — would have fired on every run that took longer than 30
milliseconds to recover, which is all of them. `BenchmarkMetricsTest` asserts
the conversion with a 45-second RTO, not just the names.
...
| `kates_benchmark_errors_total` | COUNTER, labels `run_id`, `test_type`, `phase` | kates-benchmark, kates-chaos | Tasks that reached a FAILED state, per phase. `rate()` for an error rate; the absolute value is the run's total. Registered at zero on the phase's first poll, so a healthy run reads 0 rather than nothing, and a task is counted once, by the poll that first sees it FAILED — not at the end of the run, where the meters are unregistered in the same breath and nothing would scrape it. |
...
## Integrity meters — per run, and only for a run that verified

Also registered by `BenchmarkMetrics.java`, from
`com.bmscomp.kates.domain.IntegrityResult`, and unregistered with the rest of
the run's meters. Registered **lazily**, on the first verification a run
reports: a run that never verifies integrity publishes none of these, so an
empty panel means "not verified" rather than "recovered instantly". Every plain
load test would make that second claim if these were registered eagerly at
zero.

They feed the nine `kafka:chaos:*` recording rules and, through them, the four
SLA alerts. See [the chaos rules](#the-chaos-recording-rules-kafkachaos).

**The values are in seconds.** `IntegrityResult` holds `Duration`s and exposes
milliseconds; the publisher divides by 1000. `KafkaRTOExceedsSLA` fires above
30, meaning thirty seconds, so an unconverted value would have fired it on
every run that took more than 30ms to recover.

| Series | Type & labels | Boards | What it means, and how to read it |
|---|---|---|---|
| `kates_integrity_result_producer_rto_seconds` | GAUGE, labels `run_id`, `test_type` | kates-chaos | Seconds before producers could write again after the fault. Recorded as `kafka:chaos:producer_rto_seconds`. |
| `kates_integrity_result_consumer_rto_seconds` | GAUGE, labels `run_id`, `test_type` | kates-chaos | The same for the consumer side. Tracked separately from the producer on purpose: a cluster can accept writes again well before its readers have caught up, and one recovery number hides exactly that gap. |
| `kates_integrity_result_max_rto_seconds` | GAUGE, labels `run_id`, `test_type` | *(alert only)* | The worse of the two, computed by the verifier. The recording rule reads it rather than deriving it — the derivation it used to attempt, `max without(instance) (a, b)`, is not valid PromQL and made Prometheus refuse the entire rule file. |
| `kates_integrity_result_rpo_seconds` | GAUGE, labels `run_id`, `test_type` | kates-chaos | Width of the window whose writes did not survive. With `acks=all` and a healthy ISR this should be zero, and this series is the evidence for that rather than an assumption of it. |
| `kates_integrity_result_data_loss_percent` | GAUGE, labels `run_id`, `test_type` | kates-chaos | Percentage of acknowledged records never consumed. A **percentage on both sides** — unlike the RTO figures it is not scaled. `KafkaDataLossDetected` fires above 0.1. |
| `kates_integrity_result_lost_records` | GAUGE, labels `run_id`, `test_type` | kates-chaos | The absolute count behind that percentage. A gauge, not a counter: each verification reports the totals it found over its own window rather than an increment, so a counter would either double-count or go backwards — and `rate()` over a counter that goes backwards invents a spike. |
| `kates_integrity_result_duplicate_records` | GAUGE, labels `run_id`, `test_type` | kates-chaos | Records seen more than once. Duplicates after a fault are expected without idempotence and are **not** data loss; they are published separately so a verdict can tell the two apart rather than reading every anomaly as loss. |
...
repository**. `charts/monitoring` ships the *board* with every other one;
Kyverno itself (the policies come from `charts/kates`'s
`kyvernoPolicy.enabled`), and a scrape of it, are separate.

| Series | Type & labels | Boards | What it means, and how to read it |
|---|---|---|---|
| `kyverno_admission_requests_total` | COUNTER, labels `resource_request_operation`, `request_allowed` | kyverno-security | Admission requests the webhook handled. It rises with deployments, not with violations. `request_allowed="false"` is the refused set, and it carries a zero fallback on the board because with every policy in Audit mode nothing is ever refused, the series does not exist, and *No data* would look like a broken board rather than a quiet one. That fallback is anchored to this series *unfiltered*, so it can only draw its zero while the webhook is answering and being scraped — a Kyverno nobody scrapes reads *No data*, not "nothing was blocked". Use `increase(…[1h])`. |
