...
WHAT THIS FILE DOES NOT DECIDE — see README.md for the whole list:
  ...
  * `$namespace` and `$cluster` are `query` variables resolved from the data,
    not constants the chart freezes in. `$pods` is gone: the pod-name regex it
    held said "this release" in a form no picker could offer, and since the
    PodMonitor carries kafka-common.strimziRelabelings the same pods are
    selected by `strimzi_io_cluster`.
  * the SLO section reads `mm2:*` recorded series, which exist only where the
    ...
    variants rather than a panel that renders empty. They carry the rule's own
    `cluster` label rather than `strimzi_io_cluster`; see `SLO` below.
...
from panels import (custom_var, query_var, stat, table, target, targets,
                    timeseries)
...
# `SEL` scopes every series the WORKERS publish; `SLO` scopes the recorded
# `mm2:*` series. They carry the same two values and not the same two labels,
# which is the trap this board used to avoid by not selecting at all.
#
# SEL. `namespace` and `strimzi_io_cluster` are both relabelings
# charts/mirror-maker2's PodMonitor applies (kafka-common.strimziRelabelings),
# so both are on every series a worker publishes — the JMX exporter's,
# the JVM's and the embedded clients'. Strimzi's own
# Labels.generateDefaultLabels sets `strimzi.io/cluster` to the CUSTOM
# RESOURCE NAME on every operand, and the chart names the KafkaMirrorMaker2
# after the release, so `$cluster` selects exactly the pods the old
# `pod=~"<release>-mirrormaker2-.*"` regex did — without a regex only the
# chart could write.
#
# SLO. The `mm2:*` series are RECORDED by charts/mirror-maker2's PrometheusRule
# (templates/alerts.yaml), whose SLI rules aggregate with `max by (namespace)`
# and then attach `cluster: {{ include "mirror-maker2.fullname" . }}` and
# `source:` as rule labels. `strimzi_io_cluster` does not survive that
# aggregation and is not re-added, so the recorded series must be selected by
# `cluster=` — a different label name holding the same string, because the
# rule's `cluster` and the operand's `strimzi.io/cluster` are both the CR name.
SEL = 'namespace="$namespace", strimzi_io_cluster="$cluster"'
SLO = 'namespace="$namespace", cluster="$cluster"'

# The series both variables are populated from. The rebalance bean exists from
# the moment a worker joins the group, which a connector- or task-scoped series
# does not: a release whose connectors have all failed still fills the pickers.
# dashboards/kafka-connect uses the same anchor for the same reason.
...
def variables(variant: str) -> list[dict]:
    ...
    These were `constant`s the chart injected, which froze an imported copy of
    the board to whoever generated it — and `$pods` was worse than frozen, a
    pod-name regex built from the release name that no picker could ever
    offer. They are ordinary `query` variables now, resolved from the data.
    The chart still pre-selects the release's own namespace and cluster by
    writing `current`, so a chart-delivered board opens on its own release;
    an imported one falls through to the first value Prometheus returns.
    """
    return [
        query_var(
            "namespace",
...
def _health() -> Row:
    return Row("Is it up, is it lagging, is it erroring", [
        stat(
            ...
            "this does not.",
            targets(("count(kafka_connect_worker_metrics_task_count{%s})" % SEL, "workers")),
...
def _task_states() -> Row:
    ...
    queries = [
        target("sum by (connector) (kafka_connect_worker_metrics_connector_%s_task_count"
               "{%s})" % (metric, SEL), legend, instant=True, fmt="table",
               ref=chr(ord("A") + i))
...
def _translation() -> Row:
    return Row("Offset translation — what a failover would resume from", [
        ...
        timeseries(
            ...
            targets(("sum by (connector) (rate("
                     "kafka_connect_source_task_metrics_source_record_write_total"
                     '{%s,connector=~".*MirrorCheckpointConnector"}[5m]))' % SEL,
                     "{{connector}}")),
            w=8, unit="reqps"),
        timeseries(
...
def _errors() -> Row:
    return Row("Errors, retries and dead letters", [
        ...
        timeseries(
            ...
            "records are gone.",
            targets(("sum(rate(kafka_connect_task_error_metrics_total_retries{%s}[5m]))" % SEL,
                     "retries"),
                    ("sum(rate("
                     "kafka_connect_task_error_metrics_deadletterqueue_produce_requests"
                     "{%s}[5m]))" % SEL, "dead-letter writes"),
                    ("sum(rate("
                     "kafka_connect_task_error_metrics_deadletterqueue_produce_failures"
                     "{%s}[5m]))" % SEL, "dead-letter write failures")),
            w=6, unit="reqps"),
        stat(
            "Last error",
            ...
            targets(("max by (connector) ("
                     "kafka_connect_task_error_metrics_last_error_timestamp{%s} != 0)" % SEL,
                     "{{connector}}")),
...
def _slo() -> Row:
    return Row("Replication SLO", [
        timeseries(
            ...
            "happening. This row selects `cluster=` rather than "
            "`strimzi_io_cluster=`: the SLI rules aggregate with "
            "`max by (namespace)`, which drops the scrape's labels, and the "
            "PrometheusRule then attaches `cluster` itself. Same string — the "
            "KafkaMirrorMaker2's name — under a different label." % LAG_MS,
            targets(("mm2:slo_replication_latency:error_ratio_rate1h{%s}" % SLO,
                     "1h — {{source}}"),
                    ("mm2:slo_replication_latency:error_ratio_rate5m{%s}" % SLO,
                     "5m — {{source}}")),
            w=8, unit="percentunit", min_value=0, max_value=1,
        ...
        timeseries(
            ...
            "exporter's names.",
            targets(("mm2:tasks_running:ratio{%s}" % SLO, "{{source}}")),
            w=8, unit="percentunit", min_value=0, max_value=1),
        timeseries(
            ...
            targets(("mm2:replication_latency_ms:max{%s}" % SLO,
                     "lag — {{source}}"),
                    ("mm2:checkpoint_latency_ms:max{%s}" % SLO,
                     "translation — {{source}}")),
            w=8, unit="ms"),
    ])
...
def _workers() -> Row:
    return Row("Workers", [
        timeseries(
            ...
            targets(('sum by (pod) ((jvm_memory_used_bytes{area="heap",%s} or '
        ...
        timeseries(
            ...
            targets(("sum by (pod, gc) (rate(jvm_gc_collection_seconds_sum{%s}[5m]))"
                     % SEL, "{{pod}} / {{gc}}")),
        ...
        timeseries(
            ...
            targets(("min(kafka_connect_worker_rebalance_metrics_"
                     "time_since_last_rebalance_ms{%s})" % SEL, "since last rebalance")),
            w=6, unit="ms"),
        timeseries(
            ...
            targets(("sum(kafka_connect_worker_metrics_connector_count{%s})" % SEL,
                     "connectors"),
                    ("sum(kafka_connect_worker_metrics_task_count{%s})" % SEL, "tasks")),
            w=12, unit="short"),
        timeseries(
            "Startup failures",
            ...
            targets(("sum(rate("
                     ...
                     "{%s}[5m]))" % SEL, "connector startup failures/s"),
                    ("sum(rate(kafka_connect_worker_metrics_task_startup_failure_total"
                     "{%s}[5m]))" % SEL, "task startup failures/s")),
            w=12, unit="reqps"),
    ])
    ...
    return Row("The path a record takes — source fetch, target produce", [
        timeseries(
            ...
            targets(("sum by (topic) (kafka_consumer_fetch_manager_records_consumed_rate"
                     '{%s,client_id=~".*replication-consumer"})' % SEL, "{{topic}}")),
            w=6, unit="reqps"),
        timeseries(
...
def _per_source(identity: bool) -> Row:
    ...
    if identity:
        ...
        lag_expr = ("max by (topic) ("
                    "kafka_connect_mirror_source_connector_replication_latency_ms_max"
                    '{%s,topic=~"$source[%s].*"})' % (SEL, SEP))
        lag_description = (
            "Selected by the replicated topic prefix: under a prefixing policy "
            "the alias IS the first segment of the target topic name. Under an "
            "identity replication policy the replicated topic keeps its source "
            "name, there is no prefix to match, and this panel stays empty — "
            "set add.source.alias.to.metrics on the connector to get a real "
            "source label back.")
    ...
    return Row("Source: $source", [
        timeseries(
            ...
            targets(("sum by (connector) (rate("
                     "kafka_connect_source_task_metrics_source_record_write_total"
                     '{%s,connector=~"$source->.*"}[5m]))' % SEL, "{{connector}}")),
            w=6, unit="reqps"),
        timeseries(
