...
from panels import barchart, piechart, stat, table, target, targets, timeseries
...
def build(variant: str = "default") -> list[Row]:
    return [Row("", [
        ...
        stat(
            "Admission — Blocked",
            "Requests an Enforce policy refused. `or vector(0)` is deliberate: "
            "with every policy in Audit mode nothing is ever blocked, the series "
            "does not exist, and without the fallback this panel would read No "
            "data — which looks like a broken board rather than a quiet one.",
            targets(('sum(increase(kyverno_admission_requests_total'
                     '{request_allowed="false"}[1h])) or vector(0)', "Blocked")),
