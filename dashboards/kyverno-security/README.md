...
update in the cluster. Delivered by `charts/kates` when
`kyvernoPolicy.enabled` and `kyvernoPolicy.grafanaDashboard` are both set and
the cluster has the `kyverno.io/v1` API — the ConfigMap goes to
`kyvernoPolicy.grafanaDashboardNamespace` (default `monitoring`), which is
where the Grafana sidecar watches.
...
carries `or vector(0)` deliberately: with every policy in Audit mode nothing
...
- the ConfigMap is delivered to `kyvernoPolicy.grafanaDashboardNamespace`
  (`monitoring`), because that is where the sidecar looks;
...
`charts/kates/files/dashboards/kyverno-security.json` — `--check` fails on a
hand-edited copy.
