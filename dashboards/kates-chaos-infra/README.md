...
Delivery is no argument either way any more — since kates-chaos 2.2.0 every
board ships from `charts/monitoring` — so the content argument above is the
whole reason this stays its own board: a health-of-the-platform board folded
into a what-did-the-fault-do board answers neither question cleanly.
...
|---|---|---|---|
| `$namespace` | hidden `constant` | where the execution plane runs | `litmus` |
| `$operator_deployment` | hidden `constant` | the Litmus operator Deployment's name | `litmus`, litmus-core's own fullname |

Neither is derivable from the data, so both are hidden `constant` template
variables. A Litmus under another name or namespace changes them in
`board.py` (then `scripts/gen-dashboards.py`) when the change is everyone's,
or on the installed board when it is one cluster's — kates-chaos 2.2.0
removed the values keys that used to fill them per release.
...
`charts/monitoring/dashboards/kates-chaos-infra.json` — `--check` fails
