...
def _header() -> list[dict]:
    return [
        P.stat(
            ...
            "zero instead of visibly blank — and the thresholds are red at "
            "zero, because no worker up is the outage rather than the "
            "resting state.",
        ...
        P.stat(
            ...
            "and the count falls on its own. That is what the zero fallback "
            "is for, and why it is anchored to the workers' rebalance series: "
            "'no failed connectors' and 'no workers being scraped' must not "
            "both draw a green zero.",
        ...
        P.stat(
            ...
            "KafkaConnectTaskFailed fires on it after 2 minutes. The zero "
            "fallback is anchored — see 'Failed connectors'.",
            P.targets(zero_if_live(
                'sum(kafka_connect_connector_task_status{status="failed", %s})' % SEL)),
            unit="short", thresholds=GREEN_RED_1,
        ),
        P.stat(
            "Tasks running",
        ...
        P.stat(
            ...
            "is KafkaConnectErrorsLogged. The zero fallback is anchored — see "
            "'Failed connectors'.",
            P.targets(zero_if_live(
