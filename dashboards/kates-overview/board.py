...
def zero_if_live(expr: str) -> str:
    ...
    return P.or_zero(expr, "%s{%s}" % (ANCHOR, APP))
...
def _header() -> list[dict]:
    return [
        ...
        P.stat(
            ...
            "are on Kates — Trend & Regression. The zero fallback is anchored "
            "to the process' uptime: a release that has completed nothing "
            "yet reads 0, and one nobody is scraping reads No data.",
            P.targets(zero_if_live(
                "sum(kates_tests_completed_total{%s})" % APP)),
        ...
        P.stat(
            ...
            "nothing has to call the API for a run to proceed. That is why "
            "the zero fallback is anchored rather than bare — on this panel "
            "a true zero and a dead scrape look identical, and only the "
            "anchor tells them apart.",
            P.targets(zero_if_live(
                "sum(rate(http_server_requests_seconds_count{%s}[5m]))" % APP)),
