...
`charts/kates/templates/grafana-dashboard.yaml` loads
`files/dashboards/kates-overview.json` with `.Files.Get` and wraps it in the
same ConfigMap it always did — same name
(`<fullname>-grafana-dashboard`), same labels, same data key
(`kates-dashboard.json`) — so an existing install keeps the same object and
the Grafana sidecar keeps picking it up.
