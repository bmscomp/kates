...
The `api` kind exists for the debrief tasks, whose ground truth is the report of a `broker-kill-recovery` template run: `POST /api/disruptions/templates/{id}` runs the template synchronously, stores its report once, when the run has ended, and returns it, and no kates command runs a template. Until #207, a plan or playbook the launcher accepted never had its outcome stored: `DisruptionLauncher` saved it as a second insert under the id of its RUNNING placeholder, which failed, so the record stayed RUNNING and `kates disruption run` timed out after 20 minutes. #207 made that save replace the placeholder, and left template and scheduled runs, which insert a new id once, as they were. The debrief setups still need the template run: it kills the broker named by `brokerId`, which each of them captures at run time, and returns when the run has ended. `kates disruption run` reads a plan only from `--config <file>`, and no setup step kind writes a file, let alone one filled with a capture; `kates disruption playbook run` takes no target, and no shipped playbook kills one chosen broker.
...
- **A disruption record left at RUNNING** (`sre-stale-running-disruption`, retired). It asked whether Kates was running anything while the record of a plan that had ended still said RUNNING (mcp caveat activity-disruption-rows, as it read then). #207 made the backend store a plan's outcome, and mark a report that a stopped backend left RUNNING as INTERRUPTED when it starts again, so on a backend with #207 the task's setup cannot produce its premise: its last step, which checks it, fails and stops `run.py setup` partway through the list. It was retired on 2026-09-26, the day the list was fixed, before any setup or trial ran on it, so `fixed_at` did not change. Two run directories had already frozen the 21-task list, without running setup or a trial; such a run takes the 20-task list the next time it runs, and says so (a run takes a changed list until it has set up a task, computed an answer or scheduled a trial). A run that did set up or trial the 21-task list keeps it and is refused (exit 2), since the harness no longer accepts it: start a new `--run-id`. Both lists carry the same `fixed_at`; the report tells them apart by the list's SHA-256, which it prints next to `fixed_at`.
