...
func TestDeployCommand_MonitoringWiresEveryScrape(t *testing.T) {
	...
	want := map[string][]string{
		...
		"helm upgrade --install kates charts/kates":                     {"--set metrics.serviceMonitor.enabled=true", "--set metrics.prometheusRule.enabled=true"},
...
func TestDeployCommand_NoMonitoringPassesNoScrapeFlags(t *testing.T) {
	...
	for _, c := range cmds {
		for _, banned := range []string{"Monitor.enabled=", "Monitors.enabled=", "alerts.enabled=", "chaosAlerts", "exporter.enabled="} {
