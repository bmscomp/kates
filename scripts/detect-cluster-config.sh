...
if kubectl get deployment -A -o jsonpath='{.items[*].metadata.name}' 2>/dev/null | grep -Eq "strimzi.*operator"; then
    STRIMZI_DEPLOYED=true
    STRIMZI_NS=$(kubectl get deployment -A -o custom-columns=NS:.metadata.namespace,NAME:.metadata.name --no-headers 2>/dev/null | awk '/strimzi.*operator/ {print $1}' | head -n1)
...
# Kind nodes report the host's full CPU/RAM, inflate the profile.
# Cap broker replicas to 1-per-AZ on Kind to avoid overcommitting.
...
# Build controller pools YAML
CONTROLLER_POOLS_YAML=""
if [ "${ZONE_COUNT}" -ge 3 ]; then
    for i in 0 1 2; do
        ZONE="${ZONES[$i]}"
        POOL_NAME="controllers-az$((i+1))"
        CONTROLLER_POOLS_YAML+="  - name: ${POOL_NAME}
    zone: \"${ZONE}\"
    replicas: 1
    storageSize: ${CONTROLLER_STORAGE}
    storageClass: \"${DEFAULT_SC}\"
"
    done
elif [ "${ZONE_COUNT}" -eq 2 ]; then
...
"
    ...
    CONTROLLER_POOLS_YAML+="  - name: controllers-az3
    zone: \"\"
    replicas: 1
    storageSize: ${CONTROLLER_STORAGE}
    storageClass: \"${DEFAULT_SC}\"
"
    ...
    CONTROLLER_POOLS_YAML+="  - name: controllers-az1
    zone: \"${ZONES[0]}\"
    replicas: 1
    storageSize: ${CONTROLLER_STORAGE}
    storageClass: \"${DEFAULT_SC}\"
...
"
    ...
    CONTROLLER_POOLS_YAML+="  - name: controllers
    zone: \"\"
...
# ── Extract Cluster DNS Domain ──
CLUSTER_DOMAIN=$(kubectl get configmap coredns -n kube-system -o jsonpath='{.data.Corefile}' 2>/dev/null | grep -oE "kubernetes [a-zA-Z0-9.-]+" | awk '{print $2}' | head -n1)
...
strimzi-kafka-operator:
  kubernetesServiceDnsDomain: ${CLUSTER_DOMAIN}
...
controllerDefaults:
  resources:
    requests:
      memory: ${CONTROLLER_MEM_REQ:-$BROKER_MEM_REQ}
      cpu: ${CONTROLLER_CPU_REQ:-$BROKER_CPU_REQ}
    limits:
      memory: ${CONTROLLER_MEM_REQ:-$BROKER_MEM_REQ}
      cpu: ${CONTROLLER_CPU_REQ:-$BROKER_CPU_REQ}
