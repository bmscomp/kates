...
public class AdvisorService {
    ...
    public List<Recommendation> analyze(TestRun run) {
        ...
        if (spec.getBatchSize() > 0 && spec.getBatchSize() <= 16384 && avgThroughput > 10000) {
            rules.add(new Recommendation("HIGH",
        ...
        if (("1".equals(acks) || "0".equals(acks)) && spec.getReplicationFactor() >= 3) {
            ...
            rules.add(new Recommendation(sev,
                    String.format("acks=%s with replicationFactor=%d risks data loss", acks, spec.getReplicationFactor()),
                    "Use acks=all for data durability with high replication",
                    String.format("replication=%d provides redundancy, but acks=%s bypasses it",
        ...
        if ("none".equals(compression) || compression == null || compression.isEmpty()) {
            if (avgThroughput > 20000) {
                ...
                rules.add(new Recommendation("MED",
                        "No compression — consider enabling for network efficiency",
                        "compression=lz4 adds negligible CPU overhead", null));
            }
            ...
            rules.add(new Recommendation("OK",
                    String.format("compression=%s is optimal for this workload", compression),
                    null, null));
        }
        ...
        if (spec.getPartitions() > 0 && spec.getNumProducers() > 0) {
            ...
            if (ratio < 2) {
                rules.add(new Recommendation("MED",
                        String.format("partitions=%d with %d producers limits parallelism",
                                spec.getPartitions(), spec.getNumProducers()),
                        String.format("Increase partitions to %d (4× producers) for better distribution",
        ...
        if (clusterBrokers > 0 && spec.getReplicationFactor() > clusterBrokers) {
            rules.add(new Recommendation("HIGH",
                    String.format("replicationFactor=%d exceeds cluster size (%d brokers)",
        ...
        if (clusterBrokers > 0 && spec.getPartitions() < clusterBrokers && avgThroughput > 50000) {
            rules.add(new Recommendation("MED",
                    String.format("partitions=%d is less than broker count (%d) — underutilizing cluster",
