...
public record ChaosOutcome(
    ...
    public static ChaosOutcome success(
        ...
        return new ChaosOutcome(
                engineName, experimentName, start, end, startNanos, Duration.between(start, end), "Pass", null);
    }
    ...
    public static ChaosOutcome failure(
        ...
        return new ChaosOutcome(
                engineName, experimentName, start, end, startNanos, Duration.between(start, end), "Fail", reason);
    }

    public static ChaosOutcome skipped(String reason) {
        Instant now = Instant.now();
        return new ChaosOutcome("none", "none", now, now, System.nanoTime(), Duration.ZERO, "Skipped", reason);
    }
}
