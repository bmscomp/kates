...
public record IntegrityEvent(long timestampMs, String type, String detail) {
    ...
    public static IntegrityEvent crcFailure(int partition, long sequence) {
        return new IntegrityEvent(
                System.currentTimeMillis(), CRC_FAILURE, "partition=" + partition + " seq=" + sequence);
    }
    ...
    public static IntegrityEvent lostRange(long fromSeq, long toSeq, long count) {
        return new IntegrityEvent(
                System.currentTimeMillis(), LOST_RANGE, "from=" + fromSeq + " to=" + toSeq + " count=" + count);
    }
