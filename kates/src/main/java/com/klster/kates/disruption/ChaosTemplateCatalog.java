...
public class ChaosTemplateCatalog {
    ...
    public List<TemplateInfo> listTemplates() {
        return List.of(
                ...
                new TemplateInfo("network-partition-split-brain",
                        ...
                        "network", "CRITICAL", 240),
                new TemplateInfo("disk-pressure",
    ...
    private DisruptionPlan brokerKillRecovery(Map<String, Object> ov) {
        ...
        DisruptionPlan plan = basePlan("Broker Kill & Recovery",
                "Kill broker-" + brokerId + " and measure recovery time");
    ...
    private DisruptionPlan networkPartitionSplitBrain(Map<String, Object> ov) {
        ...
        DisruptionPlan plan = basePlan("Network Partition (Split Brain)",
                "Isolate broker-" + brokerId + " via NetworkPolicy");
        plan.setMaxAffectedBrokers(1);
    ...
    private DisruptionPlan diskPressure(Map<String, Object> ov) {
        ...
        DisruptionPlan plan = basePlan("Disk Pressure",
                "Fill broker-" + brokerId + " disk to " + fillPct + "%");
        plan.setMaxAffectedBrokers(1);
    ...
    private DisruptionPlan rollingRestartZeroDowntime(Map<String, Object> ov) {
        DisruptionPlan plan = basePlan("Rolling Restart (Zero-Downtime)",
                "Sequentially restart all brokers with grace period");
        plan.setMaxAffectedBrokers(-1);
    ...
    private DisruptionPlan leaderElectionStorm(Map<String, Object> ov) {
        ...
        DisruptionPlan plan = basePlan("Leader Election Storm",
                "Trigger rapid leader elections on partition " + partition);
        plan.setMaxAffectedBrokers(1);
    ...
    private DisruptionPlan consumerIsolation(Map<String, Object> ov) {
        ...
        DisruptionPlan plan = basePlan("Consumer Group Isolation",
                "Isolate consumers connected to broker-" + brokerId);
        plan.setMaxAffectedBrokers(1);
    ...
    private DisruptionPlan cascadingBrokerFailure(Map<String, Object> ov) {
        DisruptionPlan plan = basePlan("Cascading Broker Failure",
                "Kill brokers 0 and 1 sequentially");
    ...
    private DisruptionPlan cpuSaturation(Map<String, Object> ov) {
        ...
        DisruptionPlan plan = basePlan("CPU Saturation Attack",
                "Stress broker-" + brokerId + " CPU with " + cores + " cores");
        plan.setMaxAffectedBrokers(1);
    ...
    private static int intOr(Map<String, Object> ov, String key, int def) {
        ...
        if (v instanceof String s) { try { return Integer.parseInt(s); } catch (NumberFormatException e) { return def; } }
        return def;
    }
