...
public class StrimziStateTracker {
    ...
    public record ReplicationHealth(
            int totalPartitions, int underReplicatedPartitions, int offlinePartitions, boolean healthy) {}
     ...
     */
    public Duration measureRecoveryTime(
            String namespace, String kafkaCluster, Instant disruptionStart, Duration timeout) {
        ...
        LOG.warn("Kafka CR '" + kafkaCluster + "' did not recover within timeout (" + timeout.toSeconds()
    ...
    boolean isKafkaReady(String namespace, String kafkaCluster) {
        try {
            ...
            Map<String, Object> status =
                    (Map<String, Object>) kafka.getAdditionalProperties().get("status");
            if (status == null) {
                return false;
            }

            List<Map<String, Object>> conditions =
                    (List<Map<String, Object>>) status.getOrDefault("conditions", Collections.emptyList());
