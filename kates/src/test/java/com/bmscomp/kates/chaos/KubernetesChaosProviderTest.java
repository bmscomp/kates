...
public class KubernetesChaosProviderTest {
    ...
    void testPodKill() throws ExecutionException, InterruptedException, TimeoutException {
        ...
        Pod pod = new PodBuilder()
                .withNewMetadata().withName("test-pod").withNamespace("default").addToLabels("app", "test").endMetadata()
        ...
        Pod deletedPod = client.pods().inNamespace("default").withName("test-pod").get();
    ...
    void testCpuStress() throws ExecutionException, InterruptedException, TimeoutException {
        Pod pod = new PodBuilder()
                .withNewMetadata().withName("broker-0").withNamespace("default").addToLabels("app", "kafka").endMetadata()
                .withNewSpec().addNewContainer().withName("kafka").withImage("kafka").endContainer().endSpec()
                .build();
        client.pods().inNamespace("default").resource(pod).create();
        ...
        Pod updatedPod = client.pods().inNamespace("default").withName("broker-0").get();
        assertNotNull(updatedPod);
        assertEquals(1, updatedPod.getSpec().getEphemeralContainers().size());
        assertEquals("chaos-cpu-stress", updatedPod.getSpec().getEphemeralContainers().get(0).getName());
    }

    @Test
    void testIoStress() throws ExecutionException, InterruptedException, TimeoutException {
        Pod pod = new PodBuilder()
                .withNewMetadata().withName("broker-1").withNamespace("default").addToLabels("app", "kafka2").endMetadata()
                .withNewSpec().addNewContainer().withName("kafka").withImage("kafka").endContainer().endSpec()
                .build();
        client.pods().inNamespace("default").resource(pod).create();
        ...
        Pod updatedPod = client.pods().inNamespace("default").withName("broker-1").get();
        assertNotNull(updatedPod);
        assertEquals(1, updatedPod.getSpec().getEphemeralContainers().size());
        assertEquals("chaos-io-stress", updatedPod.getSpec().getEphemeralContainers().get(0).getName());
    }
