...
class AdvisorResourceTest {
    ...
    void analyzeReturnsRecommendations() {
        ...
        Mockito.when(advisorService.analyze(run))
                .thenReturn(List.of(new AdvisorService.Recommendation(
                        "HIGH", "Increase batch.size", "Set batch.size=65536", "Current value too low")));

        var response = RestAssured.given()
                .when()
                .get("/api/tests/run-1/advisor")
                .then()
                .statusCode(200)
                .extract()
                .body()
    ...
    void analyzeReturns404ForMissingRun() {
        ...
        RestAssured.given().when().get("/api/tests/missing/advisor").then().statusCode(404);
    }
}
