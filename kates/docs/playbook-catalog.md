...
`targetAll: true` is what makes this a zone failure: without it, a label-targeted fault hits one random pod the selector matches. With it, every matching pod is killed, and the safety guard counts each broker among them against `maxAffectedBrokers` — a zone holding more than three brokers is rejected before anything is killed. A KRaft controller in the zone is killed too, but it is not a broker, so it is not counted. The direct Kubernetes backend kills them all at once. The Litmus backend hands them to `pod-delete` as one `TARGET_PODS` list, which it runs with `SEQUENCE=serial`, so it deletes them one at a time.
...
The playbook does not look up the active controller. `targetBrokerId: 0` aims the fault at the broker whose pod name ends in `-0` among the pods labelled `strimzi.io/component-type=kafka`. `targetBrokerId` picks among brokers only, so it never picks a dedicated KRaft controller (`strimzi.io/broker-role=false`): if node 0 is one, the step falls back to the first broker, and the dry run warns about it. Node 0 is the active controller only if it also has the controller role and leads the metadata quorum when the step runs; `kafka-metadata-quorum.sh --bootstrap-server <broker>:9092 describe --status` prints the quorum's `LeaderId`, so check it before you run the playbook.

With the direct Kubernetes backend, the partition is a NetworkPolicy that denies all ingress and egress for that pod, held for `chaosDurationSec: 60` and then deleted; the Litmus backend runs `pod-network-partition` instead. To isolate a different broker, post the same step to `POST /api/disruptions` with `targetBrokerId` changed. To isolate a dedicated controller, name its pod in `targetPod` instead, such as `krafter-controllers-alpha-3` on the default cluster. Built-in playbooks take no parameters.

### What to Look For

The isolated node is a broker, so clients of the partitions it leads see errors until the controller moves leadership to another replica. If you isolate a dedicated controller through `targetPod` instead, existing producers and consumers should continue working normally during the 60-second partition — the data plane is separate from the control plane. The real test is what happens when the partition heals, which the 90-second observation window covers. The controller needs to reconcile its state with the rest of the cluster, which may involve metadata log catchup and potentially re-electing leaders.
...
The critical question is whether clients see any errors while the brokers roll. Kates does not grade that for you: the playbook sets no SLA, and a plan's `sla` cannot check `maxErrorRate` either, because a plan's metrics capture has no error rate (see [Defining Your SLA](disruption-guide.md#defining-your-sla)). Watch your producers' and consumers' error counts while the brokers roll. If the error rate stays at zero, your rolling restart is truly zero-downtime. If it does not, investigate your controlled shutdown settings, readiness probe configuration, or producer retry policies.
