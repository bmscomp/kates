...
echo -e "${GREEN}Deploying LitmusChaos from local charts (offline mode)...${NC}"

# Check if local chart exists
if [ ! -d "${LITMUS_CHART_DIR}" ]; then
    echo -e "${RED}Error: Litmus chart not found at ${LITMUS_CHART_DIR}${NC}"
...
# Install LitmusChaos from local chart
echo -e "${GREEN}Installing LitmusChaos Operator from local chart...${NC}"
helm upgrade --install chaos "${LITMUS_CHART_DIR}" \
  --namespace litmus \
  --values config/litmus-values-offline.yaml \
  --wait
...
echo ""
echo "LitmusChaos chaos engineering platform is now installed (offline mode)."
echo ""
echo "Next steps:"
