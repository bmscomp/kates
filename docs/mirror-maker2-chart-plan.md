...
> **Status note — 2026-09-19.** The chart this plan introduced has since moved on. It is **0.8.0**, its `appVersion` tracks Kafka **4.3.1** (Strimzi **1.2.0**, not the 1.1.0 / 4.3.0 pair quoted below), and since 0.8.0 it is built on the `kafka-common` library chart — a `file://` dependency, so `helm dependency build charts/mirror-maker2` now comes before any render from a checkout. Current reference: [charts/mirror-maker2/README.md](../charts/mirror-maker2/README.md) and [charts/kafka-common/README.md](../charts/kafka-common/README.md).
