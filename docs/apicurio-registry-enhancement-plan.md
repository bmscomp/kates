...
> **Status note — 2026-09-19.** One premise has moved: the `apicurio-registry` KafkaUser and its ACLs are **no longer in `kafka-cluster`'s base values**. Since kafka-cluster 1.0 they belong to its `platform` profile, so P0-1's "SCRAM user, ACLs, Secret already provisioned there" now means the Kafka release was installed with `-f charts/kafka-cluster/values-platform.yaml` (or `profile: platform`); a base install renders no platform users at all. The journal topics are unaffected — this chart provisions them itself. Everything else below still reads true. Current reference: [charts/kafka-cluster/README.md](../charts/kafka-cluster/README.md) and [docs/kafka-cluster-1.0-upgrade.md](kafka-cluster-1.0-upgrade.md).
