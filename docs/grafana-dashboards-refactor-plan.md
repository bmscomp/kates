...
*Resolved:* the extraction step substitutes. It reads each variable's `current`
(falling back to `query` for a `constant`, where the two are the same thing) and
still refuses an expression carrying a `$` afterwards — so when `$namespace`
stopped being a constant and `$cluster` joined it, the gate followed rather than
silently detaching. The fixture's series gained `strimzi_io_cluster` and a sixth
case: a second release in the same namespace, which a namespace-only selector
would have summed into this one's answer.
