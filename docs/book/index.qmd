...
**You are reading the Kates Definitive Guide** — from performance theory and chaos engineering to security, deployment, and operations, with three reference chapters and four appendices.
:::
...
- `kates` commands assume the CLI is installed and configured with a context that carries the API key, set up by the Quick Start in [Introduction](01-introduction.md#quick-start)
- `curl` and `grpcurl` examples assume the API is forwarded to `localhost:30083` and read the key from `$KATES_API_KEY`, exported as shown in [REST API Reference](11-api-reference.md#authentication)
