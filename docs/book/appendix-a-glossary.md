...
| **NetworkPolicy** | Kubernetes resource that controls pod-to-pod network traffic. Policies add up: a connection that any policy selecting the pod allows gets through, so a deny-all closes only what no other policy opens. In the kafka namespace, the policy Strimzi generates admits every source to a client listener until the listener carries `networkPolicyPeers` (see [Security & Compliance](17-security.md#network-policies)) |
