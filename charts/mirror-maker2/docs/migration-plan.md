...
| | Detail |
|---|---|
| Metrics, alerts, dashboards | On before the first record, so lag has history rather than a spot reading. The [migration board](dashboards.md#the-migration-board) — delivered by `charts/monitoring` — is the board for the window itself: go/no-go, the drain and its ETA, translated consumer positions, the cutover freeze |
