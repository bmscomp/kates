...
| | Detail |
|---|---|
| Metrics, alerts, dashboards | On before the first record, so lag has history rather than a spot reading. [`dashboard.migration.enabled`](dashboards.md#the-migration-board) is the board for the window itself: go/no-go, the drain and its ETA, translated consumer positions, the cutover freeze |
